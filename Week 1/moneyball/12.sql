...
SELECT first_name,last_name FROM players p WHERE id IN
        (SELECT pe.player_id FROM performances pe
        JOIN salaries s ON s.player_id = pe.player_id AND pe.year = s.year
        WHERE pe.year = 2001 AND H > 0
